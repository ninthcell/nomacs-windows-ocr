...
macro(NMC_FINDQT)
    ...
    find_package(
        Qt${QT_VERSION_MAJOR}
        REQUIRED
        COMPONENTS Widgets Network LinguistTools PrintSupport Concurrent Gui Svg
    )
...
macro(NMC_CREATE_TARGETS)
    ...
    if(MSVC)
        if(${NUM_ADDITONAL_DLLS} GREATER 0)
            foreach(DLL ${ADDITIONAL_DLLS})
                add_custom_command(
                    TARGET ${PROJECT_NAME}
                    POST_BUILD
                    COMMAND ${CMAKE_COMMAND} -E copy ${DLL} ${NOMACS_BUILD_DIRECTORY}/$<CONFIGURATION>/plugins/
                )
            endforeach()
        endif()
        ...
        set_target_properties(
            ${PROJECT_NAME}
            PROPERTIES RUNTIME_OUTPUT_DIRECTORY_DEBUG ${NOMACS_BUILD_DIRECTORY}/Debug/plugins/
        )
        set_target_properties(
            ${PROJECT_NAME}
            PROPERTIES RUNTIME_OUTPUT_DIRECTORY_RELEASE ${NOMACS_BUILD_DIRECTORY}/Release/plugins/
        )
        set_target_properties(
            ${PROJECT_NAME}
            PROPERTIES RUNTIME_OUTPUT_DIRECTORY_RELWITHDEBINFO ${NOMACS_BUILD_DIRECTORY}/RelWithDebInfo/plugins/
        )
        set_target_properties(
            ${PROJECT_NAME}
    ...
    elseif(UNIX)
        set_target_properties(
            ${PROJECT_NAME}
            PROPERTIES LIBRARY_OUTPUT_DIRECTORY ${NOMACS_BUILD_DIRECTORY}/nomacs-plugins
        )
        install(TARGETS ${PROJECT_NAME} RUNTIME LIBRARY DESTINATION ${CMAKE_INSTALL_LIBDIR}/nomacs-plugins)
        set_property(
            TARGET ${PROJECT_NAME}
